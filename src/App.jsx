...
const EXERCISE_MODES = {
  basic:       { label:'Rechnen',           desc:'Plus und Minus',           emoji:'🧮', color:'#E74C3C' },
  zahlenmauer: { label:'Zahlenmauer',       desc:'Oberer Stein = Summe',     emoji:'🧱', color:'#8E44AD' },
  zahlenhaus:  { label:'Zahlenhaus',        desc:'Paare ergeben das Dach',   emoji:'🏠', color:'#E67E22' },
  verdoppeln:  { label:'Verdoppeln',        desc:'Gleiche Zahl + gleiche Zahl', emoji:'🪞', color:'#27AE60' },
  erganzen:    { label:'Auf 10 ergänzen',   desc:'Verliebte Zahlen!',        emoji:'❤️', color:'#C0392B' },
  zahlenfolge: { label:'Zahlenfolge',       desc:'Was kommt als Nächstes?',  emoji:'🔢', color:'#2980B9' },
}
...
function zahlenmauernQ() {
  ...
  const all  = [
    {layer:0,idx:0,val:base[0]},{layer:0,idx:1,val:base[1]},{layer:0,idx:2,val:base[2]},
    {layer:1,idx:0,val:mid[0]}, {layer:1,idx:1,val:mid[1]},
    {layer:2,idx:0,val:top},
  ]
  const m = all[rnd(0,5)]
  return { type:'zahlenmauer', base, mid, top, missing:m, answer:m.val, choices:choices(m.val,3) }
}

// Zahlenhaus: roof number split into floor pairs
function zahlenhausQ() {
  ...
  return { type:'zahlenhaus', roof, floors, questionFloor:fi, hideRight, answer:ans, choices:choices(ans,3) }
}

// Verdoppeln: n + n = ?
function verdoppelnQ() {
  const n = rnd(1,10)
  return { type:'verdoppeln', n, answer:n*2, choices:choices(n*2, 4) }
}

// Ergänzen zu 10: n + ? = 10  (verliebte Zahlen)
function erganzenQ() {
  const n = rnd(1,9)
  return { type:'erganzen', n, answer:10-n, choices:choices(10-n, 3) }
}

// Zahlenfolge: sequence with one missing number
function zahlenfolgeQ() {
  const step = [1,2,5][rnd(0,2)]
  const dir  = Math.random() > 0.3 ? 1 : -1
  const start = dir > 0 ? rnd(1,10) : rnd(10,18)
  const seq = Array.from({length:5}, (_,i) => start + i*step*dir)
  if (seq.some(n => n < 0 || n > 20)) return zahlenfolgeQ()
  const hideIdx = rnd(1,3)
  return { type:'zahlenfolge', seq, hideIdx, step:step*dir, answer:seq[hideIdx], choices:choices(seq[hideIdx], step+1) }
}
...
function buildQuestions(grade, mode) {
  ...
  const gen = { zahlenmauer:zahlenmauernQ, zahlenhaus:zahlenhausQ, verdoppeln:verdoppelnQ, erganzen:erganzenQ, zahlenfolge:zahlenfolgeQ }
  return Array.from({length:TOTAL_QUESTIONS}, gen[mode] || (() => basicQ(grade)))
}
...
function beep(type) {
  try {
    ...
    const play = (freq,t,dur,vol=.22,wave='sine') => {
...
function Lives({count}) {
  return (
    <div className="lives">
      {[1,2,3].map(i => <span key={i} className={i<=count?'heart-on':'heart-off'}>{i<=count?'❤️':'🖤'}</span>)}
    </div>
  )
}
...
function Zahlenmauer({q, feedback}) {
  ...
  const Stone = ({layer,idx}) => {
    ...
    return (
      <div className={`stone ${empty?'stone-empty':''} ${feedback&&empty?`stone-fb-${feedback}`:''}`}>
        {empty?'?':val}
      </div>
    )
  }
  return (
    <div className="zahlenmauer">
      <div className="zm-row"><Stone layer={2} idx={0}/></div>
      <div className="zm-row"><Stone layer={1} idx={0}/><Stone layer={1} idx={1}/></div>
      <div className="zm-row"><Stone layer={0} idx={0}/><Stone layer={0} idx={1}/><Stone layer={0} idx={2}/></div>
    </div>
  )
}
        ...
        {floors.map((fl,i) => {
          ...
          return (
            <div key={i} className={`zh-floor ${isQ?'zh-floor-active':''}`}>
              <div className={['zh-cell', isQ&&!hideRight?'zh-empty':'', isQ&&!hideRight&&feedback?`zh-fb-${feedback}`:''].join(' ')}>
...
function VerdoppelnDisplay({q}) {
  return (
    <div className="vd-wrap">
      <div className="basic-expr">
        <span className="be-num">{q.n}</span>
        <span className="be-op" style={{color:'#27AE60'}}>+</span>
        <span className="be-num">{q.n}</span>
        <span className="be-op" style={{color:'#27AE60'}}>=</span>
        <span className="be-blank">?</span>
      </div>
      <div className="vd-dots-row">
        {[0,1].map(g => (
          <div key={g} className="vd-group">
            {Array.from({length:q.n}).map((_,i) => <span key={i} className="vd-dot">●</span>)}
          </div>
        ))}
      </div>
    </div>
...
function ErganzenDisplay({q}) {
  return (
    <div className="eg-wrap">
      <div className="basic-expr">
        <span className="be-num">{q.n}</span>
        <span className="be-op" style={{color:'#C0392B'}}>+</span>
        <span className="be-blank">?</span>
        <span className="be-op" style={{color:'#C0392B'}}>=</span>
        <span className="be-num">10</span>
      </div>
      <div className="ten-frame">
        {Array.from({length:10}).map((_,i) => (
          <div key={i} className={`tf-cell ${i<q.n?'tf-filled':'tf-empty'}`}>
            {i<q.n?'●':''}
          </div>
        ))}
      </div>
...
function ZahlenfolgeDisplay({q}) {
  const {seq,hideIdx,step} = q
  const stepStr = step>0 ? `+${step}` : `${step}`
  return (
    <div className="zf-wrap">
      <p className="zf-label">Welche Zahl fehlt?</p>
      <div className="zf-seq">
        {seq.map((n,i) => (
          <div key={i} className="zf-item">
            <div className={`zf-box ${i===hideIdx?'zf-box-empty':''}`}>
              {i===hideIdx?'?':n}
            </div>
            {i<seq.length-1 && <div className="zf-arrow">{stepStr}</div>}
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="screen mode-screen">
      ...
      <div className="mode-grid">
        {Object.entries(EXERCISE_MODES).map(([key,m]) => (
          <button key={key} className="mode-tile" onClick={() => {beep('click'); onSelect(key)}}
            style={{'--mc':m.color}}>
    ...
    <div className="screen game-screen" style={{background:cfg.bg}}>
      ...
      {streak>=3 && (
        <div className="streak-badge">
          🔥 {streak}× Kombo! {streak>=7?'⚡':streak>=5?'🚀':''}
        </div>
      )}
      ...
      <div className={`question-card ${feedback==='correct'?'qc-correct':feedback==='wrong'?'qc-wrong':''}`}
        ...
        {q.type==='basic'        && <BasicDisplay q={q} color={cfg.color}/>}
        {q.type==='zahlenmauer'  && <Zahlenmauer  q={q} feedback={feedback}/>}
        {q.type==='zahlenhaus'   && <Zahlenhaus   q={q} feedback={feedback}/>}
        {q.type==='verdoppeln'   && <VerdoppelnDisplay q={q}/>}
        {q.type==='erganzen'     && <ErganzenDisplay  q={q}/>}
        {q.type==='zahlenfolge'  && <ZahlenfolgeDisplay q={q}/>}
      ...
      <p className="game-hint">
        {q.type==='basic'       ? (isChoice?'Tippe auf die richtige Antwort!':'Tippe die Antwort ein!') :
         ...
         q.type==='zahlenhaus'  ? '🏠 Was gehört ins Haus? (links + rechts = Dach)' :
         q.type==='verdoppeln'  ? '🪞 Verdoppele die Zahl!' :
         q.type==='erganzen'    ? '❤️ Ergänze auf 10!' :
...
function BasicDisplay({q, color}) {
  return (
    <div className="basic-expr">
      <span className="be-num">{q.a}</span>
      <span className="be-op" style={{color}}>{q.op}</span>
      <span className="be-num">{q.b}</span>
      <span className="be-op" style={{color}}>=</span>
      <span className="be-blank">?</span>
    </div>
  )
}
...
function ResultScreen({grade, exerciseMode, score, correct, lives, onReplay, onHome}) {
  const cfg    = GRADE[grade]
  const modeLabel = EXERCISE_MODES[exerciseMode]?.label || ''
  ...
  return (
    <div className="screen result-screen" style={{background:cfg.bg}}>
      ...
      {grade===1 && modeLabel && <p className="rs-mode-label">{EXERCISE_MODES[exerciseMode]?.emoji} {modeLabel}</p>}
      <Stars count={stars}/>
      ...
      {stars<3 && (
        <p className="rs-tip">
          {correct<4?'Üb weiter – du schaffst das! 💪':'Noch ein bisschen mehr und du bekommst 3 Sterne! ⭐⭐⭐'}
        </p>
      )}
      <div className="result-actions">
        ...
        <button className="btn-secondary"
          onClick={() => {beep('click'); onHome()}}>
          {grade===1 ? '← Übung wählen' : '🏠 Zurück'}
        </button>
      </div>
    </div>
  )
...
export default function App() {
  ...
  const handleAnswer = useCallback((raw) => {
    ...
    const ok  = ans === questions[currentQ].answer
    if (ok) {
      const ns = streak+1
  ...
  // Exit game: grade 1 → mode select, others → welcome
  const handleExit = useCallback(() => {
    clearTimeout(timer.current)
    setFeedback(null)
    setScreen(grade===1 ? 'modeSelect' : 'welcome')
  }, [grade])
    ...
    <div className="app-root">
      ...
      {screen==='result' && grade && (
        <ResultScreen
          ...
          onHome={() => setScreen(grade===1 ? 'modeSelect' : 'welcome')}
        />
      )}
    </div>
  )
